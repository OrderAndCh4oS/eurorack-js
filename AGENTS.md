...
**Available modules**: `midi-cv` (mono MIDI-CV) · `midi-4` (4-voice poly MIDI) · `midi-cc` (CC to CV) · `midi-clk` (MIDI clock) · `midi-drum` (drum pads to triggers) · `clk` (clock) · `div` (divider) · `swing` (swing clock) · `burst` (burst generator) · `gate-delay` (dual delayed gates) · `cascade` (rank-nested rhythm generator) · `lfo` · `quad-lfo` (quadrature LFO) · `nse` (noise) · `vco` · `wavetable` (wavetable oscillator) · `complex-vco` (through-zero/harmonic oscillator) · `ensemble-vco` (16-voice scale oscillator) · `sh` (sample&hold) · `quant` (quantizer) · `arp` (arpeggiator) · `changes` (scale-aware harmonic sequencer) · `refrain` (Seed-CV-addressable phrase-form sequencer with selective mutation, Hold, and Recall) · `seq` (sequencer) · `prob-seq` (probability sequencer with ratchets and conditional gates) · `seq-switch` (sequential switch) · `euclid` (euclidean rhythm) · `logic` (AND/OR gates) · `mult` (signal splitter) · `matrix` (matrix mixer) · `joystick` (XY controller) · `pluck` (plucked string voice) · `vcf` · `lpg` (low pass gate) · `formant` (formant filter) · `resbank` (resonator bank) · `fold` (wavefolder) · `ring` (ring mod) · `vocoder` (12-band channel vocoder) · `rnd` (random) · `envf` (envelope follower) · `func` (function generator) · `adsr` · `vca` · `atten` (attenuverter) · `slew` · `mix` · `dly` (delay) · `tape` (tape delay) · `verb` (reverb) · `shimmer` (pitch-feedback reverb) · `chorus` · `phaser` · `flanger` · `crush` (bit crusher) · `loop` (minimal looper) · `granulita` (granular chord) · `db` (VU meter) · `pwm` (pulse width mod) · `turing` (random looping seq) · `ochd` (8x LFO) · `chaos` (continuous Lorenz chaotic CV) · `cv-rec` (dual CV/gate recorder) · `cmp2` (window comparator) · `comp` (compressor/limiter) · `kick` · `snare` · `hat` · `scope` · `spectrum` (FFT analyzer) · `plot` (waveform plotter) · `spectrogram` (freq over time) · `rec` (WAV recorder) · `pitch-track` (monophonic audio-to-continuous-1 V/oct tracker with held Pitch and validity Gate) · `out`
...
When the core module graph changes, bump the same worklet graph revision in `audio/worklet-engine.js`, `audio/worklet/processor.js`, and `audio/worklet/core-plugin.js`. Use `<date>-<serial>-<digest>`, where `<digest>` is the first eight hex characters of SHA-256 over the ordered `alias:import-path:module-id` tuples joined by newlines. This invalidates the browser's cached static worklet graph; a contract test rejects revision drift and stale graph fingerprints.
