...
|-------------|--------|--------------|--------------|----------|--------------|------------------|------|
| `refrain` | done | research/modules/refrain.md | Refrain | sequencer | Phrase-aware deterministic variation, mutation, freeze, recall, voltage-addressed seed auditioning, and high-level musical control for Changes, Cascade, Arp, and other clocked modules without requiring scene programming. | Gamut/Multi Repetitor manuals and development notes, Marbles/Metropolix/Tetrapad-style controlled variation, voltage-addressed deterministic randomness, loop and mutation workflows, phrase-quantized control, performance demos and reviews. | High |
| `changes` | done | research/modules/changes.md | Changes | sequencer | Scale-aware, voice-led chord progressions exposed as immediately useful pitch sequences, chord roots, and harmonic-change events. | Gamut Repetitor, Opp Ned, Sinfonion/Harmonàig and related manuals/demos, diatonic harmony, cyclic voice-leading algorithms, performance-oriented progression interfaces. | Medium |
| `cascade` | done | research/modules/cascade.md | Cascade | clock | Four rank-nested rhythm outputs controlled by fill and rotation, so fill changes add or remove events without rearranging the underlying phrase. | Multi/Numeric/Zularic Repetitor, Mutable Grids/Marbles source and manuals, ranked density masks, Euclidean contrasts, multi-output rhythm performance demos. | Medium |
...
| `chaos` | implementing | research/modules/chaos.md | Chaotic CV Generator | modulation | Non-repeating modulation from Lorenz/logistic-style systems, distinct from random and LFO modules. | Nonlinear dynamics papers, Sloths/chaos module manuals, chaos synth zines/articles, oscilloscope demos. | Medium |
| `vocoder` | spec-ready | research/modules/vocoder.md | Filter-Bank Vocoder | effect | Analysis/synthesis envelope transfer for speech-like filtering and carrier/modulator patches. | Classic vocoder articles, EMS/Sennheiser history, DAFX filter-bank vocoder material, hardware and plugin demos. | High |
...
| `cv-rec` | implementing | research/modules/cv-rec.md | CV Recorder | modulation | Records and loops control gestures as voltages, enabling automation lanes and reusable performed modulation. | Planar/Frames-style modulation recording, voltage memory modules, looper UX reviews, interpolation strategies. | Medium |
...
| `prob-seq` | implementing | research/modules/prob-seq.md | Probability Sequencer | sequencer | Step sequencing with per-step probability, ratchets, skips, and conditional gates rather than fixed deterministic playback. | Elektron conditional trigs, Metropolis/Metropolix, Varigate, probability rhythm papers, performance demos. | Medium |
...
| `pitch-track` | spec-ready | research/modules/pitch-track.md | Pitch Tracker | utility | Converts monophonic audio pitch to 1V/oct CV and gate, enabling audio-controlled synth patches. | Autocorrelation/YIN pitch detection papers, guitar synth history, envelope/pitch follower modules, latency discussions. | High |
...
| `shimmer` | spec-ready | research/modules/shimmer.md | Shimmer Reverb | effect | Pitch-shifted feedback reverb for octave clouds and ambient harmonic tails, beyond the current basic reverb. | Eventide pitch/reverb history, shimmer reverb algorithms, allpass/FDN references, pedal and module demos. | High |
